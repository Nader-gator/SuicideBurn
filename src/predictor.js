  ...
  for (i = 1; i < ship.history.length - 2; i++) {
    ...
    if (i < ship.history.length - 5) {
      if (
        tooLateHere(
          ship.sHistory[i + 5],
          ship.history.length - i + 5,
          ship,
          surface,
          [ship.history[i + 5][0], ship.history[i + 5][1]],
          realShip.angle
        )
      ) {
        break;
...
const tooLateHere = (
  ...
  if (stepsForHStop > stepsRemaining || stepsforVstop > stepsRemaining) {
    const verticlCollisionStopped = verticalSecondarySimulation(
      speed[0],
      speed[1],
      coords[0],
      coords[1],
    ...
    const horizontalCollisionStopped = horizontalSecondarySimulation(
      speed[0],
      speed[1],
      coords[0],
      coords[1],
...
const horizontalSecondarySimulation = (
  hSpeed,
  vSpeed,
  x,
  y,
